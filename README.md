...
**The Insights page (`/insights`) aggregates server-side.** The full dataset spans ~52 paginated REST requests, and my first implementation fetched exactly that — which promptly got rate-limited by the API's Cloudflare (HTTP 429), especially from shared serverless egress IPs. The fix uses GraphQL aliases: every page of a resource is requested in a single query (`p1: characters(page: 1) {...} p2: ...`), collapsing the whole aggregation to 4 HTTP requests that ask for exactly the fields the charts need. A server-only service (`lib/api/analytics.ts`) computes every aggregate once and caches the result for an hour via `unstable_cache`, with a one-shot backoff retry on 429 as a safety net. The client receives one typed payload, renders charts (Recharts), and never talks to the API itself.
